...
_TRACE_HEAT_PATH_PROMPT = (
    "You are looking at a MOTION-HEAT visualization from a fixed golf tee "
    "camera: one video frame with accumulated motion overlaid in color "
    "(blue = pixels that moved only briefly, green/yellow/orange/red = "
    "progressively more constant motion). Small WHITE CIRCLES mark brief "
    "transient motion dots — candidate golf-ball positions.\n"
    "Task: trace the GOLF BALL'S FLIGHT PATH. The ball-in-flight signature "
    "is a tight chain of white circles in a smooth arc leaving the golfer: "
    "climbing from near the club/impact zone, possibly bending over an "
    "apex and descending, or exiting the frame. Two near-parallel strands "
    "close together are the same flight (frame jitter) — trace between "
    "them.\n"
    "NOT the ball: the fan/arc of streaks hugging the golfer's body (club "
    "sweep), dense dot clusters over water/pool ripple or foliage, and "
    "scattered isolated dots that don't form a smooth chain.\n"
    "Reply with JSON only:\n"
    '{"found": true|false, "points": [{"x": <int>, "y": <int>}, ...], '
    '"reason": "<one short sentence>"}\n'
    "points: 4-10 waypoints in PIXEL coordinates of this image, ordered "
    "along the flight from launch (nearest the golfer) to the last point "
    "you can follow. The image dimensions are stated in the user message. "
    "If there is no clear ball path, found=false with an empty points "
    "list."
)


def trace_ball_path_on_heat(image_path, model: str | None = None) -> dict:
    """One Claude vision call: trace the ball's flight path as waypoints on
    a motion-heat composite with the transient dots ringed in white. The
    operator A/B-tested this: shown the overlay, the model narrated the
    correct launch→apex→exit path (and called out the club fan / pool /
    foliage as non-ball) on a clip where the geometric corridor found
    nothing.

    The image is downscaled for the API when wide; returned waypoints are
    scaled back to the ORIGINAL image's pixel space. Returns {available,
    found, points: [{x, y}, ...], reason}. Never raises."""
    out = {"available": False, "found": None, "points": [], "reason": None}
    if not HAS_ANTHROPIC or not os.environ.get("ANTHROPIC_API_KEY"):
        out["reason"] = "ANTHROPIC_API_KEY not set"
        return out
    if not HAS_CV:
    ...
    try:
        ...
        if img is None:
            out["reason"] = "could not read image"
            return out
        oh, ow = img.shape[:2]
        scale = 1.0
        if ow > 1400:
            scale = 1400.0 / ow
            img = cv2.resize(img, (1400, max(1, int(round(oh * scale)))))
        sh, sw = img.shape[:2]
        ok_enc, jpg = cv2.imencode(
            ".jpg", img, [int(cv2.IMWRITE_JPEG_QUALITY), 85],
        )
        if not ok_enc:
            out["reason"] = "could not encode image"
            return out
    except Exception as exc:  # noqa: BLE001
        out["reason"] = f"image prep failed: {exc}"
        return out
    try:
        client = _anthropic_client()
        resp = client.messages.create(
            ...
            messages=[{"role": "user", "content": [
                {
                    "type": "text",
                    "text": (
                        f"The image is {sw}x{sh} pixels. JSON only."
                    ),
                },
                {"type": "image", "source": {
                    "type": "base64", "media_type": "image/jpeg",
    ...
    out["available"] = True
    out["reason"] = parsed.get("reason")
    if isinstance(parsed.get("found"), bool):
        out["found"] = parsed["found"]
    pts = []
    inv = 1.0 / scale
