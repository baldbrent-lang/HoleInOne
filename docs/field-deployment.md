...
The Pi HQ Camera (Sony IMX477) is **not limited to 30 fps** — that's
usually just a software default. Its sensor modes trade resolution for
speed:

| Resolution | Max fps | Use |
|---|---|---|
| 4056×3040 (full 12 MP) | ~10 fps | too slow for motion |
| 2028×1520 (4:3, ~3 MP) | ~40 fps | |
| **2028×1080 (16:9, ~2 MP)** | **~50 fps** | **capture target** |
| 1332×990 (binned, ~1.3 MP) | ~120 fps | too low-res downrange |

**Capture at 1080p / 50 fps.** That roughly halves how far the ball
travels between frames versus 30 fps — a smoother traced flight line and
less motion blur per frame — while keeping enough resolution to resolve
the ball downrange. It's also a clear step up from the ~29 fps GoPro
stutter this architecture replaced.

Pi 5 caveats:
- **The Pi 5 encodes video in software** — it dropped the Pi 4's
  hardware H.264 encoder. Sustained high-fps recording is therefore
  CPU-bound; fine for the short 10–12 s clips this design records
  (bursty, not continuous), but verify the record-while-detecting load
  on the bench.
- **Rolling shutter** — the IMX477 is a rolling-shutter sensor, so a
  ball moving fast *close* to the lens (right off the tee) can skew
  slightly. If beta footage shows this, the Raspberry Pi Global Shutter
  Camera (IMX296) removes it but is only 1.6 MP (less downrange reach);
  test the HQ cam at 1080p50 first.
- **CSI cable** — the Pi 5's camera port is the narrower 22-pin
  connector, so the HQ camera's bundled 15-pin cable won't fit; use a
  22-pin → 15-pin adapter cable.
...
3. **Field install** — Mount tee enclosure ~25 yd behind the tee
   ...
   is minimal, high enough to see over walking golfers). (If you plan to
   run the green-camera measurement features — distance-to-pin, longest
   putt — mount the **green** enclosure higher and angled down onto the
   green instead, and mount it rigidly; see *Green-camera measurement
   features* below. The tee camera stays low.)
...
#### Green-camera measurement features (distance-to-pin, longest putt)

Both features below reuse one piece of infrastructure: a **one-time
homography** that maps green-plane pixels → real-world feet. Because the
green camera never moves, you calibrate that perspective distortion away
once per mount and reuse it all season.

**Prerequisite — elevated, angled-down green camera.** Accuracy depends
on the camera's angle to the green. A low/grazing side view (~0–5° down)
foreshortens badly: pixels-per-foot changes drastically with depth, so
small pixel errors become large distance errors, and balls are easily
occluded by green contours. An **elevated mount angled down onto the
green (~20–30°+ depression)** gives a far more uniform scale and much
less occlusion — the closer to overhead, the closer pixel distance maps
linearly to ground distance. This overrides the "~3 ft off the ground"
MVP mount **for the green camera only**; the tee camera stays low. Mount
the elevated green cam **rigidly** — the homography assumes a fixed
camera, so wind sway corrupts the measurement.

**Calibration (once per green camera):**
1. Place 4+ markers at measured positions on the green; record their
   pixel coordinates; compute homography `H` (image → green-plane feet).
2. Each round, detect the **flagstick base** (the cup itself is usually
   too small / occluded) and map it through `H` for the hole's ground
   position. The flagstick's known height (7 ft regulation, 8 ft some
   courses) doubles as a scale sanity-check.

**Distance-to-pin estimate** (replaces the earlier flagstick
pixel-to-feet sketch):
- Detect the ball at rest by its **ground-contact point**, map through
  `H`, and take the straight-line distance to the hole. Output in feet.
- Caveat: the planar homography assumes a flat green — mild slope is
  fine within ~20–30 ft of the pin; heavy contour adds error.
- Simpler fallback with no markers: use the flagstick's pixel height for
  its known 7 ft to get pixels-per-foot *at the hole*, then convert the
  ball-to-flag pixel gap — only roughly right when the ball is close to
  the pin.

**Longest putt made:**
- A putt = a ball **at rest on the green that then rolls along the
  surface** (distinct from an approach shot arcing in from the air).
  Record its start position (last resting pixel → `H`).
- "Made" = the ball reaches the **cup** (flagstick base) and
  **disappears there** rather than rolling past or stopping short.
- Distance = **straight-line** start-to-hole via `H` (golf convention —
  "a 40-footer" — not the curved roll path the ball traveled). Longest =
  max over all made putts in the round/day.
- **Architectural addition:** putts happen minutes after the approach,
  outside the tee-triggered green-recording window. The green cam must
  therefore **monitor continuously while players are on the green** (or
  person-triggered, like the tee-box ROI), which raises its active duty
  cycle and compute versus the tee-triggered-only model — factor into
  the power budget (bounded: only while a group is on the green).
- Challenges: golfer occlusion at address (the elevated angle helps),
  filtering non-putt motion (walking, marking, lifting the ball), and
  tracking the slow, subtle roll of a putt.

All of the above are deferred features sketched in chat but not
committed.
